document.addEventListener("DOMContentLoaded", () => {
  ...
  sortiertePositionen.forEach(position => {
    if (gruppiert[position]) {
      renderPosition(position, gruppiert[position], container);
      delete gruppiert[position]; // schon verarbeitet
    }
  });

  // Danach alles andere alphabetisch
  Object.keys(gruppiert).sort().forEach(rest => {
    renderPosition(rest, gruppiert[rest], container);
  });
});
